...
public class Knight
{
    ...
    /**
     * Initialize the knight at position (0,0). 
     */
    public Knight()
    {
     ...
     *
     * @return {@link Point} object with the knight's current position.
     */
    ...
    /**
     * Checks if the current move is one move away from the first move;
     * used to check if the tour was a perfect tour at the end of the tour.
     *
     * @param board instance of {@link ChessBoard}. 
     * @return {@link Boolean} true if the last move is one move away from the first
     * move. 
     */
    public boolean isPerfectTour(ChessBoard board)
    {
        for(int i = 0 ; i < moveTypes[0].length ; i++)
        {

            // Position of the next move
            Point pos = new Point( position.x + (moveTypes[0][i]) , 
                                   position.y + (moveTypes[1][i]) ) ; 
            
                if(board.oneMoveAway(pos))
                    return true;
        }
     ...
     * @return {@link Boolean} true if the move is legal, else false.
     */
    ...
    /**
     * Move the knight to the new random position and set the tour number.
     *
